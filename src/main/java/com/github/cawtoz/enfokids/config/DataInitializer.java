...
public class DataInitializer implements CommandLineRunner {
    ...
    private void createActivitiesAndPlansIfNotFound() {
        ...
        Activity juegoOrdenRapido = findOrCreateActivity(
                "Juego de orden rapido",
                "objetos caen y hay que clasificarlos en categorías, pero las categorías cambian cada cierto tiempo. Trabaja el cambio de reglas",
                ActivityTypeEnum.DIGITAL
        );
        ...
        // Nuevo plan que contiene únicamente actividades digitales
        ActivityPlan planDigital = new ActivityPlan();
        planDigital.setTherapist(therapist);
        planDigital.setTitle("Plan digital para atención");
        planDigital.setDescription("Colección de juegos digitales para trabajar la atención, memoria de trabajo y cambio de reglas.");

        PlanDetail d1 = new PlanDetail();
        d1.setPlan(planDigital);
        d1.setActivity(juegoMemoria);
        d1.setFrequencyUnit(FrequencyUnitEnum.WEEK);
        d1.setFrequencyCount(5);
        d1.setRepetitions(2);
        d1.setEstimatedDuration(10);

        PlanDetail d2 = new PlanDetail();
        d2.setPlan(planDigital);
        d2.setActivity(juegoParejas);
        d2.setFrequencyUnit(FrequencyUnitEnum.WEEK);
        d2.setFrequencyCount(5);
        d2.setRepetitions(3);
        d2.setEstimatedDuration(20);

        PlanDetail d3 = new PlanDetail();
        d3.setPlan(planDigital);
        d3.setActivity(juegoOrdenRapido);
        d3.setFrequencyUnit(FrequencyUnitEnum.WEEK);
        d3.setFrequencyCount(3);
        d3.setRepetitions(3);
        d3.setEstimatedDuration(30);

        planDigital.getDetails().add(d1);
        planDigital.getDetails().add(d2);
        planDigital.getDetails().add(d3);

        activityPlanRepository.save(planDigital);
    }
